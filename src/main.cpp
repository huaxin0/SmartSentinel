...
int main(int argc, char* argv[])
{
    ...
    g_router["/api/user/login"] = std::bind(&UserController::login, &userController, std::placeholders::_1, std::placeholders::_2);
     // 设备列表 (注意：std::placeholders::_1 简写为 _1，需要 using namespace std::placeholders)
    g_router["/api/device/list"] = std::bind(&DeviceController::listDevices, &deviceController, _1, _2);
    
    // 设备注册
    g_router["/api/device/register"] = std::bind(&DeviceController::registerDevice, &deviceController, _1, _2);   
    // 如果你写了注册功能，可以在这里解开注释
